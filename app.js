...
// Route to handle PAN Card correction submission
app.post('/submit-correctpan-application', upload.fields([
  { name: 'pancard_proof', maxCount: 1 },
  { name: 'image', maxCount: 1 },
  { name: 'signature', maxCount: 1 },
  { name: 'documents', maxCount: 1 }
]), async (req, res) => {
  try {
      ...
      const pancard_proof = req.files['pancard_proof'] ? req.files['pancard_proof'][0].filename : null;
      ...
      const correctionPan = new CorrectionPan({
          pancard_proof, image, signature, documents, uniqueNumber
      });

      await correctionPan.save();

      res.json({ message: 'PAN Card correction application submitted successfully!' });
  } catch (error) {
      console.error('Error submitting PAN Card correction application:', error);
      res.status(500).json({ message: 'Internal Server Error' });
  }
});
...
// Route to fetch all PAN applications
app.get('/get-all-correction-pan-applications', async (req, res) => {
  try {
      const applications = await CorrectionPan.find();  // Adjust to your model and query
      res.json(applications);
  } catch (error) {
      console.error('Error fetching Correction PAN applications:', error);
